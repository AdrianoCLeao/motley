...
use engine_assets::{
    AssetId, AssetServer, MaterialData, MaterialHandle, MeshHandle, SceneExternalComponents,
...
impl ViewportRenderModule {
    ...
    pub fn render(
        ...
        sync_primary_camera_aspect(world, self.width, self.height);
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("engine-editor-viewport-encoder"),
            });

        {
            let _pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("engine-editor-viewport-clear-pass"),
    ...
    fn encode_3d_pass(
        ...
        let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("engine-editor-viewport-3d-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view,
                resolve_target: None,
                ops: wgpu::Operations {
        ...
        for draw_item in draw_items {
            ...
            let material_buffer =
                self.device
                    .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                        label: Some("engine-editor-material-uniform"),
            ...
            let material_bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("engine-editor-material-bind-group"),
    ...
    fn encode_2d_pass(
        ...
        for batch in draw_batches {
            ...
            let sprite_bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("engine-editor-sprite-bind-group"),
    ...
    fn ensure_gpu_mesh(&mut self, handle: MeshHandle, assets: &AssetServer) -> Result<()> {
        ...
        let vertex_buffer = self
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("engine-editor-mesh-vertex-buffer"),
        ...
        let index_buffer = self
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("engine-editor-mesh-index-buffer"),
    ...
    fn ensure_gpu_texture(&mut self, handle: TextureHandle, assets: &AssetServer) -> Result<()> {
        ...
        let texture = self.device.create_texture(&wgpu::TextureDescriptor {
            label: Some("engine-editor-texture"),
            size: wgpu::Extent3d {
                width,
                height,
                depth_or_array_layers: 1,
        ...
        let sampler = self.device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("engine-editor-texture-sampler"),
...
fn collect_draw_items_3d(world: &mut World, max_draw_items: usize) -> Vec<DrawItem3d> {
...
fn collect_draw_items_2d(world: &mut World, max_draw_items: usize) -> Vec<DrawItem2d> {
    let mut draw_items = Vec::new();
    let mut query = world.query_filtered::<
...
fn build_draw_batches_2d(draw_items: &[DrawItem2d]) -> Vec<DrawBatch2d> {
    let mut ranges = Vec::new();
    let mut iter = draw_items.iter().enumerate();
    let Some((_, first)) = iter.next() else {
        return ranges;
    };

    let mut current_texture = first.texture;
    let mut current_texture_id = first.texture.id();
    let mut start = 0usize;

    for (index, item) in iter {
        if item.texture.id() != current_texture_id {
            ranges.push(DrawBatch2d {
                texture: current_texture,
                start,
                end: index,
            });
            current_texture = item.texture;
            current_texture_id = item.texture.id();
            start = index;
        }
    }
...
fn sync_primary_camera_aspect(world: &mut World, width: u32, height: u32) {
    let mut query = world.query_filtered::<&mut Camera3d, With<PrimaryCamera>>();
    if let Some(mut camera) = query.iter_mut(world).next() {
        camera.set_aspect_ratio_from_viewport(width, height);
    }
}
...
fn create_depth_target(device: &wgpu::Device, width: u32, height: u32) -> DepthTarget {
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("engine-editor-depth-texture"),
        size: wgpu::Extent3d {
            width,
            height,
            depth_or_array_layers: 1,
...
fn create_pipeline_3d(device: &wgpu::Device, target_format: wgpu::TextureFormat) -> Pipeline3d {
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("engine-editor-mesh3d-shader"),
        source: wgpu::ShaderSource::Wgsl(MESH3D_SHADER.into()),
    });

    let camera_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("engine-editor-camera3d-layout"),
    ...
    let model_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("engine-editor-model-layout"),
        entries: &[wgpu::BindGroupLayoutEntry {
            binding: 0,
    ...
    let material_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("engine-editor-material-layout"),
        entries: &[
    ...
    let camera_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("engine-editor-camera3d-uniform"),
    ...
    let camera_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("engine-editor-camera3d-bind-group"),
    ...
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("engine-editor-mesh3d-pipeline-layout"),
        bind_group_layouts: &[&camera_layout, &model_layout, &material_layout],
        push_constant_ranges: &[],
    });

    let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("engine-editor-mesh3d-pipeline"),
...
fn create_pipeline_2d(device: &wgpu::Device, target_format: wgpu::TextureFormat) -> Pipeline2d {
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("engine-editor-sprite2d-shader"),
        source: wgpu::ShaderSource::Wgsl(SPRITE2D_SHADER.into()),
    });

    let camera_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("engine-editor-camera2d-layout"),
        entries: &[wgpu::BindGroupLayoutEntry {
            binding: 0,
            visibility: wgpu::ShaderStages::VERTEX,
            ty: wgpu::BindingType::Buffer {
    ...
    let sprite_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("engine-editor-sprite-layout"),
        entries: &[
            wgpu::BindGroupLayoutEntry {
                binding: 0,
                visibility: wgpu::ShaderStages::FRAGMENT,
    ...
    let camera_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("engine-editor-camera2d-uniform"),
    ...
    let camera_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("engine-editor-camera2d-bind-group"),
        layout: &camera_layout,
        entries: &[wgpu::BindGroupEntry {
            binding: 0,
            resource: camera_buffer.as_entire_binding(),
    ...
    let quad_vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("engine-editor-sprite-quad-vertex-buffer"),
        contents: bytemuck::cast_slice(quad_vertices.as_slice()),
        usage: wgpu::BufferUsages::VERTEX,
    });
    let quad_index_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("engine-editor-sprite-quad-index-buffer"),
        contents: bytemuck::cast_slice(quad_indices.as_slice()),
        usage: wgpu::BufferUsages::INDEX,
    });

    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("engine-editor-sprite2d-pipeline-layout"),
        bind_group_layouts: &[&camera_layout, &sprite_layout],
        push_constant_ranges: &[],
    });

    let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("engine-editor-sprite2d-pipeline"),
        layout: Some(&pipeline_layout),
        vertex: wgpu::VertexState {
            module: &shader,
            entry_point: Some("vs_main"),
...
fn upload_rgba8_texture(
    ...
    let upload_data = if aligned_row_bytes == row_bytes {
        pixels[..expected_size].to_vec()
    } else {
